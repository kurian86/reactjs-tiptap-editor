...
export function BubbleMenu({ editor, disabled, bubbleMenu }: BubbleMenuComponentProps) {
  ...
  const renderMenuItems = () => [
    extensionsNames.includes('columns') && !bubbleMenu?.columnConfig?.hidden ? <ColumnsBubbleMenu editor={editor} key="columns" /> : null,
    extensionsNames.includes('table') && !bubbleMenu?.tableConfig?.hidden ? (
      <TableBubbleMenu editor={editor} actions={bubbleMenu?.tableConfig?.actions} key="table" />
    ) : null,
    extensionsNames.includes('link') && !bubbleMenu?.linkConfig?.hidden ? (
      <BubbleMenuLink disabled={disabled} editor={editor} key="link" />
    ) : null,
    extensionsNames.includes(Image.name) && !bubbleMenu?.imageConfig?.hidden ? (
      <BubbleMenuImage disabled={disabled} editor={editor} key="image" />
    ) : null,
    extensionsNames.includes(ImageGif.name) && !bubbleMenu?.imageGifConfig?.hidden ? (
      <BubbleMenuImageGif disabled={disabled} editor={editor} key="imageGif" />
    ) : null,
    ...
    extensionsNames.includes('twitter') && !bubbleMenu?.twitterConfig?.hidden ? (
      <BubbleMenuTwitter disabled={disabled} editor={editor} key="twitter" />
    ) : null,
    !bubbleMenu?.floatingMenuConfig?.hidden ? <ContentMenu disabled={disabled} editor={editor} key="content" /> : null,
    !bubbleMenu?.textConfig?.hidden ? <BubbleMenuText disabled={disabled} editor={editor} key="text" /> : null,
    extensionsNames.includes('drawer') && !bubbleMenu?.drawerConfig?.hidden ? (
